...
contribute cmp = do
    ...
    utxo <- watchAddressUntil
        (Scripts.scriptAddress inst)
        (campaignCollectionDeadline cmp)
...
successfulCampaign = do
    ...
    void $ Trace.waitUntilSlot (Haskell.succ $ TimeSlot.posixTimeToSlot $ campaignDeadline theCampaign)
